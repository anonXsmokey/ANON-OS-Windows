...
$plan=[ordered]@{schemaVersion=1;source=[ordered]@{imageFile=$ImageFile;imageIndex=$ImageIndex};target=[ordered]@{firmware='UEFI';architecture='x64'};profile=$Profile;features=[ordered]@{ai=$Ai;telemetry=$Telemetry;recovery=$Recovery;restorePoint=$RestorePoint}}

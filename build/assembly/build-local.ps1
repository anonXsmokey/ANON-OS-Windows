...
$publish=Join-Path $root 'build\shell-publish';New-Item -ItemType Directory -Force -Path $publish|Out-Null
Push-Location $root
try{
 ...
 & (Join-Path $root 'build\assembly\build-iso.ps1') -WindowsIso (Resolve-Path $WindowsIso).Path -ShellPublish $publish -OutputRoot $OutputRoot -Architecture x64 -ImageIndex $ImageIndex;if($LASTEXITCODE){throw 'ISO assembly failed.'}

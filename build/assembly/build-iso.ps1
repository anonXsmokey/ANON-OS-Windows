...
# Prefer oscdimg bundled with a custom Windows ADK installation.
function Resolve-Tool([string]$Name){
  $cmd=Get-Command $Name -ErrorAction SilentlyContinue
  if($cmd){return $cmd.Source}
  if($Name -eq 'oscdimg.exe'){
    $roots=@('C:\Program Files (x86)\Windows Kits','C:\Program Files\Windows Kits','E:\Windows Kits\10\ADK')
    $candidates=@()
    foreach($root in $roots){if(Test-Path -LiteralPath $root){$candidates+=Get-ChildItem -LiteralPath $root -Filter $Name -Recurse -File -ErrorAction SilentlyContinue}}
    $preferred=$candidates|Where-Object{$_.FullName -match '\\amd64\\Oscdimg\\oscdimg\.exe$'}|Select-Object -First 1
...
$dism=(Resolve-Tool 'dism.exe');$oscdimg=(Resolve-Tool 'oscdimg.exe');$reg=(Resolve-Tool 'reg.exe')

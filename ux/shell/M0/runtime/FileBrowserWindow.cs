...
public sealed class FileBrowserWindow : Window
{
    ...
    {
        Title="ANON Files";
        var root=new Grid{Background=BackgroundBrush,Padding=new Thickness(20,20,20,20)};
        ...
        _list.ItemClick+=(_,e)=>{if(e.ClickedItem is Entry item){if(item.IsFolder)OpenFolder(item.Path);else Launch(item.Path);}};
        Grid.SetRow(_list,2);root.Children.Add(_list);Content=root;ShowHome();
    }
    private static void AddButton(Panel panel,string text,Action action){var button=new Button{Content=text};button.Click+=(_,_)=>action();panel.Children.Add(button);}
    private void ShowHome(){_currentPath=string.Empty;_path.Text="THIS PC";_list.ItemsSource=DriveInfo.GetDrives().Where(d=>d.IsReady).Select(d=>new Entry(d.Name.TrimEnd('\\'),d.Name,true)).ToList();}
    private void OpenFolder(string path){if(!Directory.Exists(path))return;_currentPath=path;_path.Text=path;var entries=new List<Entry>();try{entries.AddRange(Directory.EnumerateDirectories(path).OrderBy(x=>x).Select(x=>new Entry(Path.GetFileName(x)??x,x,true)));entries.AddRange(Directory.EnumerateFiles(path).OrderBy(x=>x).Take(500).Select(x=>new Entry(Path.GetFileName(x)??x,x,false)));}catch(UnauthorizedAccessException){entries.Add(new Entry("ACCESS DENIED",string.Empty,false));}_list.ItemsSource=entries;}
    private static void Launch(string target){if(string.IsNullOrWhiteSpace(target))return;try{Process.Start(new ProcessStartInfo{FileName=target,UseShellExecute=true});}catch{}}

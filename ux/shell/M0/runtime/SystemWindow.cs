...
public sealed class SystemWindow : Window
{
    ...
    public SystemWindow()
    {
        ...
        AddAction(actions, "WINDOWS SETTINGS", "ms-settings:", 1);
        ...
        var telemetryCard = Card();
        var telemetryStack = new StackPanel { Spacing = 12 };
        telemetryStack.Children.Add(Title("PERFORMANCE"));
        _telemetryText.Text = "CPU     —\nMEMORY  —";
        _telemetryText.FontSize = 14;
        _telemetryText.Foreground = Muted;
        telemetryStack.Children.Add(_telemetryText);
        _cpuBar.Maximum = 100;
        _cpuBar.Height = 5;
        telemetryStack.Children.Add(_cpuBar);
        _memoryBar.Maximum = 100;
        _memoryBar.Height = 5;
        telemetryStack.Children.Add(_memoryBar);
        telemetryCard.Child = telemetryStack;
        content.Children.Add(telemetryCard);

        var hardwareCard = Card();
        var hardwareStack = new StackPanel { Spacing = 10 };
        hardwareStack.Children.Add(Title("HARDWARE"));
        _hardwareText.FontSize = 13;
        _hardwareText.TextWrapping = TextWrapping.Wrap;
        _hardwareText.Foreground = Muted;
        hardwareStack.Children.Add(_hardwareText);
        hardwareCard.Child = hardwareStack;
        Grid.SetColumn(hardwareCard, 1);
        content.Children.Add(hardwareCard);

        var storageCard = Card();
        var storageStack = new StackPanel { Spacing = 10 };
        storageStack.Children.Add(Title("STORAGE"));
        _storageText.FontSize = 13;
        _storageText.TextWrapping = TextWrapping.Wrap;
        _storageText.Foreground = Muted;
        storageStack.Children.Add(_storageText);
        storageStack.Children.Add(new TextBlock
        {
            Text = "ANON does not modify storage policy in M0.",
            FontSize = 11,
            Foreground = Muted,
            TextWrapping = TextWrapping.Wrap
        });
        storageCard.Child = storageStack;
        Grid.SetRow(storageCard, 1);
        content.Children.Add(storageCard);

        var runtimeCard = Card();
        var runtimeStack = new StackPanel { Spacing = 10 };
        runtimeStack.Children.Add(Title("WINDOWS RUNTIME"));
        runtimeStack.Children.Add(new TextBlock
        {
            Text = $"Architecture  {RuntimeInformation.OSArchitecture}\n.NET           {Environment.Version}\nProcessors     {Environment.ProcessorCount}\nMachine        {Environment.MachineName}",
            FontSize = 13,
            Foreground = Muted
        });
        runtimeCard.Child = runtimeStack;
        Grid.SetColumn(runtimeCard, 1);
        Grid.SetRow(runtimeCard, 1);
        content.Children.Add(runtimeCard);

        Grid.SetRow(content, 2);
        root.Children.Add(content);

        _statusText.Text = "Telemetry active.";
    ...
    private static void AddAction(Grid grid, string label, string target, int column)
    {
        var button = new Button { Content = label, Margin = new Thickness(column == 0 ? 0 : 8, 0, 0, 0) };
        button.Click += (_, _) => Launch(target);
    ...
    private void Update()
    {
        ...
        _cpuBar.Value = snapshot.CpuPercent;
        _memoryBar.Value = snapshot.MemoryUsedPercent;
        _hardwareText.Text = $"Logical processors  {Environment.ProcessorCount}\nOS architecture     {RuntimeInformation.OSArchitecture}\nProcess architecture {RuntimeInformation.ProcessArchitecture}\nOS                  {Environment.OSVersion.VersionString}";
        _storageText.Text = string.Join("\n", DriveInfo.GetDrives().Where(d => d.IsReady).Select(d =>
            $"{d.Name.TrimEnd('\\')}   {FormatBytes(d.TotalSize - d.AvailableFreeSpace)} used / {FormatBytes(d.TotalSize)}"));
    ...
    private static void Launch(string target)
    {
        ...
        {
            // External Windows utilities are best-effort in M0.
        }
    }

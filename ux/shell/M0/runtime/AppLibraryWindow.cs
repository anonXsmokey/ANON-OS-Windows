...
public sealed class AppLibraryWindow : Window
{
    ...
    {
        const string xaml = @"<DataTemplate xmlns='http://schemas.microsoft.com/winfx/2006/xaml/presentation'><Border Padding='12,10' Margin='2' CornerRadius='8'><Grid><Grid.ColumnDefinitions><ColumnDefinition Width='42'/><ColumnDefinition Width='*'/><ColumnDefinition Width='Auto'/></Grid.ColumnDefinitions><Border Width='34' Height='34' CornerRadius='9' Background='#182238'><FontIcon Glyph='{Binding Icon}' FontSize='16' Foreground='#63A3FF'/></Border><StackPanel Grid.Column='1' Margin='12,0,16,0' VerticalAlignment='Center' Spacing='2'><TextBlock Text='{Binding Name}' FontSize='14' FontWeight='SemiBold' Foreground='#EEF2FA'/><TextBlock Text='{Binding Subtitle}' FontSize='11' Foreground='#919DB1'/></StackPanel><TextBlock Grid.Column='2' Text='›' FontSize='22' Foreground='#919DB1' VerticalAlignment='Center'/></Grid></Border></DataTemplate>";
    ...
    private IEnumerable<Entry> Discover()
    {
        ...
        foreach (var root in roots.Where(Directory.Exists).Distinct(StringComparer.OrdinalIgnoreCase))
        {
            ...
            foreach (var path in files)
            {
                ...
                var name = Path.GetFileNameWithoutExtension(path); if (string.IsNullOrWhiteSpace(name) || name.StartsWith("Uninstall", StringComparison.OrdinalIgnoreCase)) continue;
